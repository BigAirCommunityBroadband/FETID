...
switch ($cmd) {
  ...
  if (!isset($q_LinkedTables)) {
    ...
    $q_LinkedTables->start_row = 0;
    $q_LinkedTables->row_count = 50;
  ...
  if (empty($query)) { $query="`id`!='0' order by `id`"; }
  ...
  if (!$q_LinkedTables->start_row) $q_LinkedTables->start_row = 0;
  if (!$q_LinkedTables->row_count) $q_LinkedTables->row_count = 50;
